...
@interface InterstitialTest : XCTestCase
@property (nonatomic, strong) XCUIApplication *app;

@end

@implementation InterstitialTest
...
- (void)testInterstitialshow {
    XCUIElement *Label = self.app.staticTexts[@"Interstitial"];
    XCTAssertTrue(Label.exists, @"No Interstitial Type.");
    [Label tap];
    sleep(2);
    
    XCUIElement *btn = self.app.buttons[@"Load FullScreen AD"];
    XCTAssertTrue(btn.exists, @"No Load Interstitial AD btn.");
    [btn tap];
    sleep(3);
    
    XCUIElement *readly = self.app.buttons[@"Is FullScreen AD Ready"];
    XCTAssertTrue(readly.exists, @"No Load Interstitial AD  Ready btn.");
    BOOL isReady = NO;
    for(int i = 0; i < 10; i++) {
        XCUIElement *readly = self.app.buttons[@"Is FullScreen AD Ready"];
        [readly tap];
    ...
    if(isReady) {
        XCUIElement *show = self.app.buttons[@"Show FullScreen AD"];
        [show tap];
    }
    sleep(10);
}

- (void)testInterstitialClose{
    XCUIElement *Label = self.app.staticTexts[@"Interstitial"];
    XCTAssertTrue(Label.exists, @"No Interstitial Type.");
    [Label tap];
    sleep(2);
